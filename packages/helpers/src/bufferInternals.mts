...
export class BufferHelpersInternals {
	public static node_hexToBuffer(hex: UuidExport['hex']) {
	...
	public static node_bufferToHex(buffer: UuidExportBlobInput) {
		return import('node:buffer').then(({ Buffer }) =>
	...
	public static browser_hexToBuffer(hex: UuidExport['hex']) {
		/**
	...
	public static browser_bufferToHex(buffer: UuidExportBlobInput): UuidExport['hex'] {
		/**
	...
	public static browser_base64UrlToBuffer(base64url: string) {
	...
	public static node_base64ToBuffer(base64: string, isBase64Url: boolean) {
		return import('node:buffer').then(({ Buffer }) => {
	...
	public static browser_base64ToBuffer(base64: string) {
		const binaryString = atob(base64);
		const bytes = new Uint8Array(binaryString.length);
		for (const [i, char] of Array.from(binaryString).entries()) {
			bytes[i] = char.charCodeAt(0);
	...
	public static node_bufferToBase64(buffer: UuidExportBlobInput, urlSafe: boolean): Promise<string> {
		return import('node:buffer').then(({ Buffer }) =>
			// @ts-expect-error `ArrayBufferLike` is actually accepted and fine
	...
	public static browser_bufferToBase64(buffer: UuidExportBlobInput, urlSafe: boolean): string {
		// @ts-expect-error `ArrayBufferLike` is actually accepted and fine
