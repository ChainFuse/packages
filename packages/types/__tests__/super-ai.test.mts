import { SuperAi, type llmProviders } from '@chainfuse/super-ai';
...
describe('AI Response Tests', async () => {
	...
	for (const stream of [true, false]) {
		for (const llmProviderKey of allLlmProviderKeys) {
			const settings = { stream, max_tokens: 128 };

			await it(JSON.stringify({ model: llmProviderKey, ...settings }), async () => {
...
describe('AI Function Tests', async () => {
